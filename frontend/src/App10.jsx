...
import { 
  ...
  Languages, Copy, ArrowRight, Search, Globe
} from 'lucide-react';
...
const App = () => {
  ...
  return (
    // J'ai remis 'relative' ici pour être sûr que le positionnement soit correct
    <div className="flex h-[100dvh] w-full font-sans overflow-hidden text-slate-800 relative bg-kech-sand">
      ...
      {/* Main Content : J'ai remis 'bg-zellij-pattern' ICI */}
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] p-4 rounded-2xl shadow-sm ${msg.role === 'user' ? 'bg-kech-primary text-white rounded-br-none' : 'bg-white text-slate-800 rounded-bl-none shadow-md'}`}>
                {msg.content}
              </div>
            </div>
          ))}
      ...
      <div className="p-4 bg-white/90 backdrop-blur border-t">
        <form onSubmit={handleSend} className="max-w-3xl mx-auto flex gap-2">
          <input value={input} onChange={e => setInput(e.target.value)} placeholder="Posez votre question..." className="flex-1 p-3 bg-gray-100 rounded-full outline-none focus:ring-2 focus:ring-kech-secondary" />
          <button disabled={loading} className="p-3 bg-kech-secondary text-white rounded-full hover:bg-orange-700 transition"><Send size={20} /></button>
        </form>
      </div>
    </div>
  );
